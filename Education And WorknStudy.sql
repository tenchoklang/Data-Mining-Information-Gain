...
SET @Combinations = (SELECT COUNT(*) AS totalCombinations
						FROM (SELECT [Education Level], [Work/study hours]
								FROM dbo.Sheet1$
								GROUP BY [Education Level], [Work/study hours]) AS A)

DECLARE @SelectedEducationLVL int --Gets the EDUCATION-LEVEL only, between the combinations of Education and health
DECLARE @SelectedWorknStudy int --Gets the OVERALL-HEALTH only, between the combinations of Education and health
...
WHILE(@SelectedCombination < @Combinations) --Loop Through all Combinations
	BEGIN
		...
		SET @SelectedEducationLVL = (SELECT [Education Level]
										FROM dbo.Sheet1$
										GROUP BY [Education Level], [Work/study hours]
		...
		SET @Denominator  = (SELECT COUNT(*) AS Freq
								FROM dbo.Sheet1$
								WHERE [Education Level] = @SelectedEducationLVL AND [Work/study hours] = @SelectedWorknStudy
								GROUP BY [Education Level], [Work/study hours])
		...
		WHILE(@SelectedIncome < @TotalIncomeCategories)
			...
			BEGIN
				...
				IF EXISTS(SELECT *
								FROM dbo.Sheet1$
								WHERE [Education Level] = @SelectedEducationLVL AND [Work/study hours] = @SelectedWorknStudy AND [Household Income ] = @SelectedIncome)
					BEGIN
						...
						PRINT 'I ( Q:Education Level (' + CAST(@SelectedEducationLVL AS VARCHAR) +')'
							+ ' , Q:Work And Study (' + CAST(@SelectedWorknStudy AS VARCHAR) +')'
