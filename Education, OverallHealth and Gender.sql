...
SET @Combinations = (SELECT COUNT(*) AS totalCombinations
						FROM (SELECT [Education Level], [Overall health ], [Gender ]
								FROM dbo.Sheet1$
								GROUP BY [Education Level], [Overall health ], [Gender ]) AS A)
...
WHILE(@SelectedCombination < @Combinations) --Loop Through all Combinations
	BEGIN
		...
		SET @SelectedEducationLVL = (SELECT [Education Level]
										FROM dbo.Sheet1$
										GROUP BY [Education Level], [Overall health ], [Gender ]
		...
		SET @Denominator  = (SELECT COUNT(*) AS Freq
								FROM dbo.Sheet1$
								WHERE [Education Level] = @SelectedEducationLVL AND [Overall health ] = @SelectedOverallHealth AND [Gender ] = @SelectedGender
								GROUP BY [Education Level], [Overall health ], [Gender ])
		...
		WHILE(@SelectedIncome < @TotalIncomeCategories)
			...
			BEGIN
				...
				IF EXISTS(SELECT *
								...
								AND  [Gender ] = @SelectedGender AND [Household Income ] = @SelectedIncome)
					BEGIN
