...
SET @Combinations = (SELECT COUNT(*) AS totalCombinations
						FROM (SELECT [Education Level], [Overall health ], [Work/study hours]
								FROM dbo.Sheet1$
								GROUP BY [Education Level], [Overall health ], [Work/study hours]) AS A)
...
WHILE(@SelectedCombination < @Combinations) --Loop Through all Combinations
	BEGIN
		...
		SET @SelectedEducationLVL = (SELECT [Education Level]
										FROM dbo.Sheet1$
										GROUP BY [Education Level], [Overall health ], [Work/study hours]
		...
		WHILE(@SelectedIncome < @TotalIncomeCategories)
			...
			BEGIN
				...
				IF EXISTS(SELECT *
								...
								AND  [Work/study hours] = @SelectedWorkNStudy AND [Household Income ] = @SelectedIncome)
					BEGIN
